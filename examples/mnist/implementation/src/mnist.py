...
def download_data(task_args: List[str]) -> None:
    ...
    Input parameters:
        --data_dir
    """
    logger.info("Starting 'downloaddata' task")
    ...
    if not os.path.isfile(data_file):
        raise ValueError("MNIST dataset has not been downloaded - dataset file does not exist: {}".format(data_file))
    else:
        logger.info("MNIST dataset has been downloaded.")
    logger.info("The 'downloaddata' task has been completed.")
...
def main():
    ...
    try:
        ...
        if ml_box_args.mlbox_task == 'downloaddata':
            download_data(task_args)
        elif ml_box_args.mlbox_task == 'train':
            train(task_args)
        else:
