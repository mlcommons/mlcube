...
from mlcommons_box_ssh.ssh_metadata import (PythonInterpreter, SystemInterpreter, VirtualEnvInterpreter)
...
class TestSystemInterpreter(TestCase):
    ...
    def test_system_interpreter_user_config(self) -> None:
        config = {'type': 'system', 'python': 'python3.8', 'requirements': 'click==7.1.2 mlcommons-box==0.2.2'}
        self.check_state(
            config,
...
class TestVirtualEnvInterpreter(TestCase):
    ...
    def test_virtualenv_interpreter_user_config(self):
        config = {'type': 'virtualenv', 'python': 'python3.8', 'requirements': 'click==7.1.2 mlcommons-box==0.2.2',
                  'location': '/opt/mlbox_resources/environments', 'name': 'docker_runner-0.2.2'}
        self.check_state(
            config,
            PythonInterpreter.create(config)
        )
