...
class DockerRun(object):
    ...
    @staticmethod
    def execute_cmd(cmd: str, die_on_error: bool = True) -> int:
    ...
    def get_env_variables() -> dict:
        """Return dict of environmental variables that this docker runner recognizes.
        Return:
    ...
    def image_exists(image_name: str) -> bool:
        ...
        """
        return DockerRun.execute_cmd(f"docker inspect --type=image {image_name} > /dev/null 2>&1",
                                     die_on_error=False) == 0
    ...
    def configure(self):
        ...
        logger.info(cmd)
        DockerRun.execute_cmd(cmd)

    def run(self):
        """Run a box."""
        image_name: str = self.mlbox.platform.container.image
        if not DockerRun.image_exists(image_name):
